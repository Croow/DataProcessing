...
def scrape_movie_page(dom):
    ...
    # create several lists for the genres, directors, writers and actors
    genres_list = []
    directors_list = []
    writers_list = []
    actors_list = []
    ...
    for e in dom.by_tag("h1")[:1]:
        for a in e.by_tag("span")[:1]:
            title = plaintext(a.content)

    # find the year that the movie was made in
    for e in dom.by_tag("span.nobr")[:1]:
        for a in e.by_tag("a"):
            year = plaintext(a.content)

    # find the duration of the movie
    for e in dom.by_tag("div.infobar"):
        for a in e.by_tag("time"):
            duration = plaintext(a.content).replace(" min","")

        # find the genres of the movie
        for a in e.by_tag("a"):
            for i in a.by_tag("span.itemprop"):
                genres_list.append(plaintext(i.content))

    # find the directors of the movie
    for e in dom.by_attr(itemprop="director")[:1]:
        for a in e.by_tag("span.itemprop"):
            directors_list.append(plaintext(a.content))

    # find the writers of the movie
    for e in dom.by_attr(itemprop="creator")[:1]:
        for a in e.by_tag("span"):
            writers_list.append(plaintext(a.content))

    # find the actors of the movie
    for e in dom.by_attr(itemprop="actors"):
        for a in e.by_tag("span.itemprop"):
            actors_list.append(plaintext(a.content))

    # find the rating of the movie
    for e in dom.by_tag("div.titlePageSprite star-box-giga-star"):
        rating = plaintext(e.content)

    # find the numer of ratings of the movie
    for e in dom.by_tag("div.star-box-details"):
        for a in e.by_tag("a")[:1]:
            for i in a.by_tag("span")[:1]:
                n_ratings = plaintext(i.content).replace(",","")

    # transform the lists of the genres, directors, writers and actors into strings
